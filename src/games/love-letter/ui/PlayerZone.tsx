...
export const HeldCardBacks: React.FC<{playerId:string; count:number}> = ({ playerId, count }) => {
  ...
  return <HeldArea ref={anchor} aria-label={`손패 ${count}장`} $empty={count===0}>
    <HeldCards>{Array.from({length:Math.min(2,count)}).map((_,index)=><HeldBack key={index}/>)}</HeldCards>
...
const LocalZoneRoot=styled.section`position:relative;width:100%; min-width:0; max-width:100%; display:grid; grid-template-rows:34px auto; align-items:end; padding:0 6px; box-sizing:border-box;`;
const LocalIdentity=styled.div`position:absolute;left:8px;bottom:10px;width:92px;z-index:2;@media(max-width:360px){left:5px;width:80px;bottom:7px;}`;
const LocalHand=styled.div`width:100%;min-width:0;`;
const IdentityButton=styled.button<{$turn:boolean;$targetable:boolean;$selected:boolean;$eliminated:boolean;$self:boolean}>`width:100%; min-width:0; height:${p=>p.$self?'94px':'31px'}; display:flex; flex-direction:${p=>p.$self?'column':'row'}; align-items:${p=>p.$self?'flex-start':'center'}; justify-content:${p=>p.$self?'center':'initial'}; gap:4px; padding:${p=>p.$self?'8px 7px':'3px 5px'}; box-sizing:border-box; border-radius:8px; border:1px solid ${THEME.border}; background:rgba(255,253,247,.96); color:${THEME.foreground}; font:inherit; cursor:${p=>p.$targetable?'pointer':'default'}; ${p=>p.$turn&&css`border-color:${THEME.gold}; background:#fffdf3;`} ${p=>p.$targetable&&css`border:2px solid ${THEME.burgundy};`} ${p=>p.$selected&&css`background:#fff1f2;`} ${p=>p.$eliminated&&css`opacity:.5;filter:grayscale(1);`} @media(max-width:360px){height:${p=>p.$self?'82px':'31px'};padding:${p=>p.$self?'6px':'3px 5px'};} @media(max-height:650px){height:${p=>p.$self?'76px':'27px'};padding:${p=>p.$self?'5px':'2px 4px'};}`;

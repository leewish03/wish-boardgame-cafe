...
export const RoundResultModal: React.FC<RoundResultModalProps> = ({ isOpen, roundNumber, localUserId, winnerReason, targetTokens=4, isHost, onNextRound, players=[], winnerIds=[], advanceAt, canAdvanceAt, isRequesting=false, requestError, previousScores, winnerCards }) => {
  ...
  const displayName = (player: PlayerPublic) => player.id === localUserId ? '나' : player.nickname;
  return <AnimatePresence>{isOpen && <Overlay as={motion.div} initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}><Sheet as={motion.section} initial={{ y:30, opacity:0 }} animate={{ y:0, opacity:1 }} exit={{ y:30, opacity:0 }}>
    ...
    <Title><Crown size={19}/>{winners.map(displayName).join(', ') || '승자'}{winners.length > 1 ? ' 공동 승리' : ' 승리'}</Title>
    <Reason>{reasonCopy[winnerReason || ''] || winnerReason || '라운드가 종료되었습니다.'}</Reason>
    {winnerCards && winners.length > 0 ? <Reveal>공개 비교 카드 · {winners.map(p => `${p.id === localUserId ? '내 카드' : p.nickname} ${winnerCards[p.id]?.value ?? '?'}`).join(' · ')}</Reveal> : null}
    <ScoreList>{players.map(p => <ScoreRow key={p.id} $winner={winnerIds.includes(p.id)}><span>{displayName(p)}</span><strong>{previousScores ? `${previousScores[p.id] ?? p.tokens} → ` : ''}{p.tokens} / {targetTokens}</strong></ScoreRow>)}</ScoreList>
